...
public class LendTableModel extends AbstractTableModel {
    ...
    public String getColumnName(int i) {
        String colNames[] = new String[]{"ID", "Borrower Name", "Borrowed Film ID", "Borrow Date", "Return Date"};
        return colNames[i];
    }
    ...
    public Object getValueAt(int rowIndex, int columnIndex) {
        switch (columnIndex) {
            case 0:
                return lendings.get(rowIndex).getId();
            case 1:
                return lendings.get(rowIndex).getBorrowingPerson();
            case 2:
                return lendings.get(rowIndex).getBorrowedFilmID();
            case 3:
                return lendings.get(rowIndex).getDate();
            case 4:
                return lendings.get(rowIndex).getBackDate();
            default:
                return null;
        }
    }
